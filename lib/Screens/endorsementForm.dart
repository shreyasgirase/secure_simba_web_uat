...
class _MyFormState extends State<MyForm> {
  ...
  List<String> status = <String>['Single', 'Married', 'Other'];
  List<String> spCodes = <String>['124456', '124452', '124458', '124459'];
  List<String> inwardType = <String>[
    'endorsement',
  ];

  List<String> attachedDocuments = <String>[
    'RC Transferred-RC Copy',
    'RC Transferred- NOC from previous insured',
    'RC Transferred-RTO Transfer fee receipt(Mandatory for smart cards)',
    'RC not Transferred-Form 29 & 30 / RTO transfer fee receipt',
    'RC not Transferred-Sale Deed',
    'RC not Transferred-NOC from previous insured'
  ];
...
// newwwwww controllerss
  ...
  TextEditingController proposerNameController = TextEditingController();
  TextEditingController mobileNoController = TextEditingController();
  TextEditingController corrAdrressController = TextEditingController();
  ...
  TextEditingController drivingExpController = TextEditingController();
  TextEditingController paNameController = TextEditingController();
  TextEditingController paRelationController = TextEditingController();
  ...
  TextEditingController ncbController = TextEditingController();
  TextEditingController odAmountController = TextEditingController();
  TextEditingController withoutNCBController = TextEditingController();
  TextEditingController ncbAmountController = TextEditingController();
  TextEditingController totalDaysYearController = TextEditingController();
  TextEditingController recoveryAmountController = TextEditingController();
  ...
  String selectedRegNO = 'BH';
  bool isVehicleReg = false;
  ...
  String driverDateOfBirth = DateFormat('yyyy-MM-dd').format(DateTime.now());
  String paCoverDateOfBirth = DateFormat('yyyy-MM-dd').format(DateTime.now());
  String dateOfTransfer = DateFormat('yyyy-MM-dd').format(DateTime.now());
  ...
  String endorsementDate = DateFormat('yyyy-MM-dd').format(DateTime.now());
  ...
  void initState() {
    ...
    // getPolicyDetails('0000000039454084');
    ...
    setState(() {
      // ncbController.text = "20";
      // odAmountController.text = "2560";
      totalDaysYearController.text = "365";
  ...
  getInwardDetails() async {
    ...
    try {
      ...
      print(data['policy_no']);
      setState(() {
        policyNumberController.text = data['policy_no'];
      });
      print("vieww ${widget.isView}");
      ...
      setState(() {
        paymentLink = data3['paymentRequestFlag'] ?? false;
      ...
      print(data4['customer_mobile_number']);
      print(data4['customer_email_id']);
      ...
      setState(() {
        customerEmailId = data4['customer_email_id'] != null
            ? data4['customer_email_id']
            : '';
        customerMobile = data4['customer_mobile_number'] != null
            ? data4['customer_mobile_number']
            : '';
      });
      print('came here too');
  ...
  getEndorsementDetails(proposalID) async {
    ...
    try {
      ...
      var data2 = jsonDecode(decryptedData1)['ownership_transfer_details'];
      ...
      if (data['form_type'] != 'Cancellation & Refund' ||
          data['sub_type'] != 'Ownership Transfer') {
        setState(() {
      ...
      if (data['sub_type'] == 'Ownership Transfer') {
        await getEndorsementSubType(
            data['endorsement_type'], data['form_type']);
      }
      await getVersion();
      ...
      print(' experience ${data2['driving_exp']}');
      if (data['sub_type'] == 'Ownership Transfer') {
        setState(() {
          titleController.text = data2['title'] ?? '';
          proposerNameController.text = data2['proposer_name'] ?? '';
          gender = data2['gender'];
          dateOfBirth = data2['date_of_birth'];
          mobileNoController.text = data2['mobile_no'] ?? '';
          corrAdrressController.text =
              data2['corr_building_name_or_block_no'] ?? '';
          streetNameController.text = data2['street_name'] ?? '';
          localityController.text = data2['locality'] ?? '';
          cityController.text = data2['city'] ?? '';
          stateController.text = data2['state'] ?? '';
          pinCodeController.text = data2['pincode'] ?? '';
          aadharNoController.text = data2['aadhar_no'] ?? '';
          panCardNoController.text = data2['pan_card_no'] ?? '';
          emailIdController.text = data2['email_id'] ?? '';
          maritalStatus = data2['marital_status'];
          driverNameController.text = data2['driver_name'] ?? '';
          driverDateOfBirth = data2['driver_dob'];
          drivingExpController.text = data2['driving_exp'] != null
              ? data2['driving_exp'].toString()
              : '';
          driverGender = data2['driver_gender'];
          paNameController.text = data2['nominee_name'] ?? '';
          paCoverDateOfBirth = data2['nominee_dob'];
          paRelationController.text = data2['nominee_relationship'] ?? '';
          vehicleModelController.text = data2['vehicle_make_model'] ?? '';
          vehicleRegNoController.text = data2['registration_number'] ?? '';
          attachedDocument = data2['attached_document'];
          dateOfTransfer = data2['date_of_transfer'];
        });
      }
  ...
  getPolicyDetails(policyNo) async {
    ...
    Map<String, dynamic> encrpytedJson = {
    ...
    try {
      ...
      final response = await dio.post(
          'https://uatcld.sbigeneral.in/SecureApp/getPolicyDetails',
          data: encrpytedJson,
          options: Options(headers: headers));

      print(response);
      var decryptedData = aesCbcDecryptJson(response.data);
      ...
      if (appState.imdCode != null) {
        if (data['INTERMEDIARY_CODE'] != null) {
        ...
        if (data['SM_ID'] != null) {
          if (appState.employee_code.toString() == data['SM_ID'].toString()) {
            setState(() {
              codeMapped = true;
            });
          } else {
      ...
      print(data["ACTUAL_NCB_PCT"]);
      print(data["RECON_OWN_DAMAGE_PREMIUM"]);
      if (codeMapped == true) {
        setState(() {
          quoteNumberController.text = data['QUOTE_NO'] ?? '';
          customerNameController.text = data['CUSTOMER_NAME'] ??
              (data['INSURED_FIRST_NAME'] != null
          ...
          salesMobileController.text =
              data['SALES_MANAGER_MOBILE_NUMBER'] ?? '';
          // agreementCodeController.text = data['AGREEMENT_CODE'] ?? '';
          ...
          ncbController.text = data["ACTUAL_NCB_PCT"].toString();
          odAmountController.text = data["RECON_OWN_DAMAGE_PREMIUM"].toString();
          print("1319");
          String rawPoicyStartDate = data["POLICY_END_DATE"].split(" ")[0];
          DateTime parsedPoicyStartDate;
          String rawPoicyEndDate = data["POLICY_END_DATE"].split(" ")[0];
          DateTime parsedPoicyEndDate;

          try {
            // Try format: 'd/M/yyyy' — e.g. "6/8/2024"
            parsedPoicyStartDate =
                DateFormat('d/M/yyyy').parseStrict(rawPoicyStartDate);
            parsedPoicyEndDate =
                DateFormat('d/M/yyyy').parseStrict(rawPoicyEndDate);
          } catch (e) {
            parsedPoicyStartDate =
                DateFormat('yyyy-MM-dd').parseStrict(rawPoicyStartDate);
            parsedPoicyEndDate =
                DateFormat('yyyy-MM-dd').parseStrict(rawPoicyEndDate);
          }
          startDateController.text =
              DateFormat('yyyy-MM-dd').format(parsedPoicyStartDate);
          endDateController.text =
              DateFormat('yyyy-MM-dd').format(parsedPoicyEndDate);

          // productType = data[0]['lob'] ?? '';
        });
        setState(() {
          customerEmailId = data['CUSTOMER_MOBILENUMBER'] != null
              ? data['CUSTOMER_EMAILID']
              : '';
          customerMobile = data['CUSTOMER_MOBILENUMBER'] != null
              ? data['CUSTOMER_MOBILENUMBER'].length == 12
        ...
        if (data['CKYC_ID'] != null) {
          List dateArr = data['DATE_OF_BIRTH'].split('-');
          dateArr[1] = dateArr[1][0] + dateArr[1].substring(1).toLowerCase();
          String date = dateArr.join('-');

          print('date here : ${data['DATE_OF_BIRTH']}');
          ...
          setState(() {
            ckycDetails = true;
            ckycNameController.text = (data['PREFIX'] ?? '') +
                " " +
            ...
            ckycTypeController.text = data['EBAO_CUST_TYPE'] == 'IND'
                ? 'Individual'
                : 'Other than Individual';
            ckycDateController.text = kycDate;

            ckycNumberController.text = data['CKYC_ID'];
          });
          // setState(() {
          //   verifyPolicy = true;
          // });
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: const Text('Wrong Policy'),
            action: SnackBarAction(
              label: ' Cancel',
              onPressed: () {},
            )));
  ...
  // getPolicyDetailsProduction(policyNo) async {
  //   final appState = Provider.of<AppState>(context, listen: false);
  //   Map<String, dynamic> postData = {
  //     "source": "simba",
  //     "policyNumber": policyNo
  ...
  //     print("Sameer");
  //     print(response);
  //     var decryptedData = aesGcmDecryptJson(response.data);
  ...
  void resetOwnershipTransferDetails() {
    setState(() {
      titleController = TextEditingController();
      proposerNameController = TextEditingController();
      mobileNoController = TextEditingController();
      corrAdrressController = TextEditingController();
      ...
      panCardNoController = TextEditingController();
      emailIdController = TextEditingController();
      driverNameController = TextEditingController();
      drivingExpController = TextEditingController();
      paNameController = TextEditingController();
      paRelationController = TextEditingController();
      vehicleModelController = TextEditingController();
      vehicleRegNoController = TextEditingController();

      dateOfBirth = DateFormat('yyyy-MM-dd').format(DateTime.now());
      driverDateOfBirth = DateFormat('yyyy-MM-dd').format(DateTime.now());
      paCoverDateOfBirth = DateFormat('yyyy-MM-dd').format(DateTime.now());
      dateOfTransfer = DateFormat('yyyy-MM-dd').format(DateTime.now());
  ...
  uploadProposal(context) async {
    ...
    Map inwardData = {
      ...
      "instruments":
          selectedPaymentMode == "Cheque" ? finalInstruments : emptyInstruments,
    };
    ...
    Map ownershipTransferDetails = {
      "ownership_transfer_details": {
        ...
        "corr_building_name_or_block_no": corrAdrressController.text,
        ...
        "driving_exp": drivingExpController.text,
        "driver_gender": driverGender,
        "nominee_name": paNameController.text,
        "nominee_dob": paCoverDateOfBirth,
        "nominee_relationship": paRelationController.text,
        "vehicle_make_model": vehicleModelController.text,
        "registration_number": vehicleRegNoController.text,
    ...
    Map endorsementData2 = {
        ...
        "customer_type": ckycDetails == true
            ...
            : customerType == "Individual"
                ...
                : customerType == "Other than Individual"
                    ? 'other'
                    : null,
        "instruments": selectedPaymentMode == "Cheque"
            ? finalInstruments
            : emptyInstruments,
        "product_category": '',
      },
      "endorsementDetails": {
        ...
        ...details,
        if (selectedEndorsementSubType != 'Ownership Transfer')
          ...endorsementData!['endorsement'],
      },
      if (selectedEndorsementSubType == 'Ownership Transfer')
    ...
    if (_inwardType == 'endorsement' && requestType == 'true') {
  ...
  editCkyc(proposalID) async {
    var kycData = {
      ...
      "customer_type": ckycTypeController.text.toLowerCase() == 'individual'
          ? 'individual'
          : 'other',
                  ...
                  ? Positioned(
                      ...
                      child: Container(
                          ...
                          child: Stack(
                            children: [
                              Style.formContainer(
                                  context, 'Premium Calcuator', [
                                heightGap(),
                                heightGap(),
                                Style.wrap(context, children: [
                                  Style.requiredFieldLabel("", 14),
                                  Style.notRequiredFieldLabel(
                                      "Note : Endorsement date entered cannot be prior to 7 days from today’s date.",
                                      14),
                                ]),
                                heightGap(),
                                Column(
                                  children: [
                                    Style.wrap(
                                      ...
                                      children: [
                                        ...
                                        CustomInputField(
                                          ...
                                          maxLines: 1,
                                          title: "NCB %",
                                          controller: ncbController,
                                        ),
                                        CustomInputField(
                                          view: viewDetails,
                                          maxLines: 1,
                                          title: "OD Amount After NCB",
                                          controller: odAmountController,
                                        ),
                                        CustomInputField(
                                          view: viewDetails,
                                          maxLines: 1,
                                          title: "Without NCB",
                                          controller: withoutNCBController,
                                        ),
                                        CustomInputField(
                                          view: viewDetails,
                                          maxLines: 1,
                                          title: "NCB Amount",
                                          controller: ncbAmountController,
                                        ),
                                        CustomInputField(
                                          view: viewDetails,
                                          maxLines: 1,
                                          title: "Tota Days(in a year)",
                                          controller: totalDaysYearController,
                                        ),
                                        CustomInputField(
                                          view: viewDetails,
                                          maxLines: 1,
                                          title: "Recovery Amount",
                                          controller: recoveryAmountController,
                                        ),
                                        CustomInputField(
                                          view: viewDetails,
                                          maxLines: 1,
                                    ...
                                    Style.formSubmitButton("Submit", () {
                                      setState(() {
                                        premiumCalculator = false;
                                        premiumCollectedController.text =
                                ...
                                onPressed: () {
                                  setState(() {
                                    ...
                                    pendingPremiumAmountController =
                                        TextEditingController();
                                    endorsementDate = DateFormat('yyyy-MM-dd')
                                        .format(DateTime.now());
                                    resetOwnershipTransferDetails();
                                  });
                                },
                                child: const Text(
                                  'Check other Policy!',
                                ...
                                ? TextButton(
                                    onPressed: () {
                                      setState(() {
                                        ...
                                        calculatePremium(
                                            DateTime.parse(endorsementDate));
                                      });
                                    },
                                    child: const Text(
                                      'Premium Calculator!',
  ...
  void calculatePremium(date) {
    DateTime endDate = DateTime.parse(endDateController.text);
    Duration difference = endDate.difference(date);
    int daysBetween = difference.inDays + 1;
    double ncb = double.parse(ncbController.text);
    double amount = double.parse(odAmountController.text);
    double discountFactor = 1 - (ncb / 100);
    double withoutNcbAmount = amount / discountFactor;
    double ncbAmount = withoutNcbAmount - amount;
    double recoveryAmount = (ncbAmount * daysBetween) / 365;
    int roundedRecoveryAmount = recoveryAmount.round();
    double gstAmount = recoveryAmount * 0.18;
    int roundedGstAmount = gstAmount.round();
    double totalAmount = recoveryAmount + gstAmount;
    int roundedTotalAmount = totalAmount.round();

    setState(() {
      totalDaysController.text = daysBetween.toString();
      withoutNCBController.text = withoutNcbAmount.round().toString();
      ncbAmountController.text = ncbAmount.round().toString();
      recoveryAmountController.text = roundedRecoveryAmount.toString();
      gstController.text = roundedGstAmount.toString();
      totalAmountController.text = roundedTotalAmount.toString();
    });
    ...
    return DropdownWidget(
        ...
        onChanged: (val) {
          setState(() {
            ...
            selectedRefundReason = null;
            selectedRefundType = null;
            if (selectedEndorsementType == 'Ownership Transfer') {
              setState(() {
                ckycDetails = false;
              });
            } else if (selectedEndorsementType != 'Ownership Transfer' &&
                ckycNumberController.text.isNotEmpty) {
              setState(() {
                ckycDetails = true;
              });
            }
            print('endt type $selectedEndorsementType');
            print('ckycdetails $ckycDetails ');
            resetOwnershipTransferDetails();
          });
  ...
  // String selectedRegNO = 'BH'; // This should be part of your state

  Widget regNoType() {
    return SizedBox(
      width: 220, // tweak this value
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          _radioOption('BH'),
...
// }
  ...
  ownershipTransferDetails() {
    return Column(
      ...
      children: [
        Style.notRequiredFieldLabel('New Proposer Details :', 14),
        Style.wrap(context, children: [
          CustomInputField(
            required: required,
            view: view,
            ...
            validator: (value) {
              return Validators.formValidation(value, 'Title');
            },
          ),
          CustomInputField(
            required: required,
            ...
            validator: (value) {
              return Validators.nameValidation(value, 'Proposer Name');
            },
          ),
          DropdownWidget(
            required: required,
          ...
          CustomInputField(
            ...
            validator: (value) {
              return Validators.phoneNumberValidation(value);
            },
          ),
          CustomInputField(
            required: required,
            view: view,
            title: 'Corr Address Building Name/Block No',
            maxLines: 1,
            controller: corrAdrressController,
            validator: (value) {
              return Validators.formValidation(value, 'Corr Address');
            },
          ),
          CustomInputField(
            required: required,
            ...
            validator: (value) {
              return Validators.formValidation(value, 'Street Name');
            },
          ),
          CustomInputField(
            required: required,
            ...
            validator: (value) {
              return Validators.formValidation(value, 'Locality');
            },
          ),
          CustomInputField(
            required: required,
            ...
            validator: (value) {
              return Validators.formValidation(value, 'City');
            },
          ),
          CustomInputField(
            required: required,
            ...
            validator: (value) {
              return Validators.formValidation(value, 'State');
            },
          ),
          CustomInputField(
            required: required,
            ...
            validator: (value) {
              return Validators.formValidation(value, 'Pin Code');
            },
          ),
          CustomInputField(
            required: required,
          ...
          CustomInputField(
              ...
              onChanged: (value) {
                panCardNoController.value = TextEditingValue(
                    text: value.toUpperCase(),
                    selection: panCardNoController.selection);
              },
              title: 'PAN Number',
        ...
        heightGap(),
        Style.notRequiredFieldLabel('Driver Details :', 14),
        Style.wrap(context, children: [
          CustomInputField(
            required: required,
            view: view,
            ...
            validator: (value) {
              return Validators.nameValidation(value, 'Name');
            },
          ),
          DatePickerFormField(
            required: required,
          ...
          CustomInputField(
            ...
            inputFormatters: [FilteringTextInputFormatter.digitsOnly],
            validator: (value) {
              return Validators.formValidation(value, 'Driving Experience');
            },
          ),
          DropdownWidget(
            required: required,
        ...
        heightGap(),
        Style.notRequiredFieldLabel('PA Cover Nominee :', 14),
        Style.wrap(context, children: [
          CustomInputField(
            required: required,
            view: view,
          ...
          CustomInputField(
            ...
            title: 'Relationship',
            maxLines: 1,
            controller: paRelationController,
            validator: (value) {
              return Validators.formValidation(value, 'Relationship');
            },
          ),
        ]),
        heightGap(),
        Style.notRequiredFieldLabel('Vehicle Details :', 14),
        Style.wrap(context, children: [
          CustomInputField(
            required: required,
            view: view,
            ...
            validator: (value) {
              return Validators.formValidation(value, 'Vehicle Make & Model');
            },
          ),
          CustomInputField(
            required: required,
            ...
            validator: (value) {
              return Validators.formValidation(value, 'Registration Number');
            },
          ),
          DropdownWidget2(
            required: required,
            view: view,
            label: 'Attached Document Type',
            items: attachedDocuments,
            value: attachedDocument,
            onChanged: (dat) {
              setState(() {
                attachedDocument = dat;
              });
            },
          ),
          DatePickerFormField(
            required: required,
            disabled: view,
            labelText: 'Date of Transfer as per RC',
            onChanged: (DateTime? value) {
              setState(() {
                dateOfTransfer =
                    DateFormat('yyyy-MM-dd').format(value as DateTime);
